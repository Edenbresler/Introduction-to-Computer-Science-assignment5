...
public class BinaryNode<T> {
    ...
    //An auxiliary function that calculates the depth of the nodes and returns (depth of the node) * 2 spaces,and then will be printed (in the same line) the information in the node.
    private String inOrder(BinaryNode thisNode,int depth,String space ,String ans, int numOfSpace) {
    	if (left != null) {
    		depth= depth+1;
    		numOfSpace =depth *2;
    		String preData = data.toString();
        	ans=left.inOrder(thisNode.left,depth,"",ans, numOfSpace) ;
        	numOfSpace=numOfSpace-2;
        	depth= depth-1;

    	}
    	for(int i=0; i<numOfSpace ; i=i+1) {
    		space = space + " ";
    	}
    	ans = ans +space + data.toString() + "\n";
    	if (right != null) {
    		depth= depth+1;
    		numOfSpace =depth *2;
    		ans =right.inOrder(thisNode.right,depth,"",ans,numOfSpace);
    	}
    	depth= depth-1;
    	numOfSpace=numOfSpace-2;
    	return ans;
    ...
    public String toString() {
    	...
    	int init =0;
    	String space = "";
    	return inOrder(this, init, space,"",0);
